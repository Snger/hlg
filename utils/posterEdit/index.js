...
KISSY.add(function (S) {
	var S = KISSY, DOM = S.DOM, Event = S.Event, win = window, doc = document,  IE = S.UA.ie,  
		DATA_ID 		= 'data-id',
		DATA_URI 		= 'data-uri',
	...
	function posterEdit(box) { //初始化属性 
		var self = this;
		if (!(self instanceof posterEdit)) { 
        	return new posterEdit(box); 
        }  
    }
	S.mix(posterEdit.prototype,{
		/*
...
//			}); 
        ...
        _initMod: function( mod ) {
            ...
            mod.appendChild( bar );
            var id = DOM.attr(mod,'id');
			Event.on('#'+id+' .ds-bar-edit', 'click', function(evt) {
				list.showDesign(id);
			});
			if ( IE ) {
